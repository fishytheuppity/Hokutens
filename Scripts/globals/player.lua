...
function onGameIn(player, firstlogin, zoning)
    if (not zoning) then -- Things checked ONLY during logon go here.
		...
		if (player:getVar("Cylon") == 1) then
		    player:PrintToServer(string.format("%s has logged in...", player:getName()), 0x1C);
        end
		
		if (player:getVar("InJail") == 1) then
			player:setPos( -620, 11, 660, 0, 131 );
        end	
		
		player:setMod(MOD_HPHEAL, 0);
    end
    ...
    if (player:getVar("AccMode") == 1) then	
        -- Add bonus mods to the player..
        player:addMod(MOD_MACC,2500);
		player:addMod(MOD_RACC,2500);
        player:addMod(MOD_ACC,2500);
		player:addMod(MOD_EVA,500);
		player:addMod(MOD_MEVA, 300);
		player:addMod(MOD_DEF,500);
		player:addMod(MOD_MDEF, 300);		
		player:addMod(MOD_HASTE_ABILITY,300);
		player:addMod(MOD_RANGED_DELAYP,-40);		
		player:addMod(MOD_STORETP, 70);	
	end
...
function CharCreate(player)
	...
	if (isValidLS("Hokutens")) then
        if not(player:hasItem(515)) then
            player:AddLinkpearl("Hokutens");
        end
    end
